...
def Totales_Menu(Tabla):
    ...
    SI = Tabla[Tabla['PTO_RIESGO'] == 'SI'].groupby(['CVE_UNIDAD','UNIDAD','PUESTO_NOM'])['EMPLEADO'].\
         nunique().reset_index().sort_values(by=['EMPLEADO'], 
                                             ascending=False)
    SI_R  = SI.sum().iloc[3]
    CVE_UNIDAD = SI.iloc[0, 0]
    return (CVE_UNIDAD, TOTAL, SI_R, SI )
...
def Mapa_Menu (UNIDAD, GEOJSON):
    ...
    MAPA = px.choropleth_mapbox(GEOJSON, 
                                ...
                                center = {'lat': 23.798375, 'lon': -102.5821213},
                                zoom = 3.5,
                                opacity = 0.5,
...
def Puestos_Menu(Tabla):
    ...
    PASTEL.update_layout(showlegend=False, title_text="",
...
def Riesgo_Puestos_Menu (Tabla):
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("", ['#990033','#eee', '#006666'])
...
def Tabla_Denuncias(Tabla):
    def renombrar_columnas(tabla, nuevos_nombres):
        tabla.columns = nuevos_nombres
        return tabla
    
    nuevos_nombres = ['No. EMPLEADO', 'RFC CORTO', 'NOMBRE', 'CLAVE UNIDAD', 'UNIDAD','GENERAL', 'CLAVE PUESTO',
                     'PUESTO', 'OBSERVACION PREVE', 'FOLIO SIDEQUS', 'AÑO', 'EXPEDIENTE', 'MEDIO DE RECEPCIÓN',
                     'CLASIFICACIÓN DEL ASUNTO','TIPO ASUNTO','FECHA DE OCURRENCIA DE HECHOS','ADMINISTRACIÓN GENERAL DONDE OCURRIERON LOS HECHOS',
                      'UNIDAD ADMINISTRATIVA DONDE OCURRIERON LOS HECHOS', 'FECHA DE RECEPCIÓN DEL ASUNTO', 'ESTATUS DEL ASUNTO',
    ...
    header=dict(values=list(TB.columns),
                ...
                align='left'),
    cells=dict(values=TB.transpose().values.tolist(),
               fill_color='lavender',
               align='left'))
                         ])
